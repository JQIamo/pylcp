...
Here, we follow the construction of the rate equations from Ref.~\cite{Tarbutt2015} and, to a lesser extent, the Ref.~\cite{Tollett1995}.
...
\subsection{Rotation of the Hamiltonian}
...
This
\section{Examples}
In this section, we cover several intriguing examples of laser cooling techniques.  These examples described herein are a small subset of the examples contained with the {\tt pylcp} package.

\subsection{Optical pumping \& Rabi flopping}
\begin{figure}
	\center
	\includegraphics{figs/damped_rabi_flopping.pdf}
	\caption{\label{fig:rabi_flopping} Damped Rabi flopping for a  }
\end{figure}

As a first example, consider an $F=1\rightarrow F=2$ atom illuminated by $\pi$ light, starting in a stretched state.  In this configuration, optical pumping will 
...
We next show an example of optical pumping, both with the rate equations and the optical Bloch equations.  We compare this directly to Unager, et. al.~\cite{Unager1989}.

\subsection{Equilibrium forces in molasses}
Equilibrium forces in an optical molasses are most accurately 


\subsection{Sub-Doppler molasses}


\subsection{Type-II 1D molasses}


\subsection{Equilibrium force in MOTs}


\subsection{Capture velocity of 1D MOT}


\subsection{Temperatures in MOTs}
