...
\begin{figure}
	...
	\caption{\label{fig:mot_forces} Force $F$ vs. both position $x$ and velocity $v$ in a one-dimensional magneto-optical trap with laser parameters $\Delta/\Gamma=1.5$ and $s_0=1$.  The white curves show trajectories through phase space for atoms entering the MOT with different initial velocities $v_0$.}
\end{figure}

Forces in a magneto-optical trap are slightly more complex than those of optical molasses, as they also depend on position as well as velocity.  Figure~\ref{fig:mot_forces} show such forces, calculated using the heuristic equation for such a one-dimensional MOT.  The laser parameters for this calculation are $\Delta/\Gamma=-1.5$ and $s_0=1$.  The white curves show trajectories, calculated without random scattering, of atoms injected into the MOT at a position $z=-20(\hbar \Gamma/\mu_B B')$ with varying initial velocity $v_0$.  Trajectories with $v_0 < 14\Gamma/k$ are captured, whereas trajectories with $v_0\geq 14 \Gamma/k$ escape.  By calculating such trajectories and bisecting the initial velocity repeatedly, one can obtain the MOT capture velocity numerically.  Figure~\ref{fig:capture} shows the result of such a calculation along with the prediction of a simple model found in Ref.~\cite{}.  This procedure has been extended to three-dimensional MOTs with non-standard geometry and should be the subject a forthcoming paper.

\begin{figure}
	\center
	\includegraphics{figs/F0_to_F1_MOT_capture}
	\caption{\label{fig:mot_forces} Capture velocity $v_c$ vs. laser detuning $\Delta$ for $s_0=$ calculated using the heuristic equation.}
\end{figure}

This above example uses the heuristic equation, which calculates the textbook example of an $F=0\rightarrow F'=1$ atom.  Extending to larger $F$ and $F'$ in the one-dimensional case produces some intriguing results.  Consider Fig.~\ref{fig:} which shows an $F=9/2\rightarrow F'=11/2$ one-dimensional MOT with $g_F=0$ and $g_F'=1/F'$.  In this case, the restoring forces rapidly disappear around $v=0$.  This disappearance is caused by shifting of the non-resonant $m_F$ component.  Specifically, consider the highlighted line along 

We also include a calculation of the temperature of a magneto-optical trap, calculated in three dimensions with the optical Bloch equations.

Finally, we have also repeated the calculation of Ref.~\cite{} of a MOT 

Other MOT examples included with {\tt pylcp} include calculating MOT damping forces and trapping frequencies along with comparison to analytic formulas~\cite{}, the forces on both $D_1$ and $D_2$ lines of alkali atoms, the forces in the two color MOT of Ref.~\cite{}, the forces for a variety of different type-I and type-II MOTs considered in Ref.~\cite{Tarbutt2015}, and the forces involved in trapping CaF in Ref.~\cite{}. 
