...
incorporating the shift into the rotating frame into the internal Hamiltonian.  We also define $\delta^L_{n\rightarrow m, p} = (\omega_{n\rightarrow m} + \omega_p) - (\omega_{R,m} - \omega_{R,n})$.  Making the rotating wave approximation (neglecting terms oscillating at optical frequencies), we find
\begin{eqnarray}
...
Before applying the radiation reaction approximation, we must first find the equations of motion.  We must be careful about operator order, as since the $\mathbf{E}$ will be dependent on the atomic operators, it is not clear that $\mathbf{E}$ will commute with $\rho_{ij}$ in general.  Inserting Eq.~\ref{eq:d_dot_E} into Eq.~\ref{eq:heisenburg_evolution}, and using $\rho_{ij}\rho_{kl} = \rho_{il}\delta_{jk}$, where $\delta_{ij}$ is the Kronicker delta function.
\begin{eqnarray}
	\hbar\frac{\partial \rho_{ij}}{\partial t} & = & -i\left(\mathbf{d}^{nm}_{kl}\cdot\mathbf{E}^\dagger_{n\rightarrow m,p} e^{-i\delta^L_{n\rightarrow m, p}  t} \right)  \ket{k}\Braket{l|i}\Bra{j} +  \nonumber \\
	& & -i \ket{l}\Bra{k} \left((\mathbf{d}^{nm}_{kl})^\dagger \cdot \mathbf{E}_{n\rightarrow m,p}e^{i\delta^L_{n\rightarrow m, p}t}\right)\ket{i}\Bra{j} \nonumber \\
	& & +i \ket{i}\Bra{j} \left(\mathbf{d}^{nm}_{kl}\cdot\mathbf{E}^\dagger_{n\rightarrow m,p} e^{-i\delta^L_{n\rightarrow m, p}  t} \right)  \ket{k}\Bra{l} \nonumber \\
	& & +i \ket{i}\Bra{j} \ket{l}\Bra{k} \left((\mathbf{d}^{nm}_{kl})^\dagger \cdot \mathbf{E}_{n\rightarrow m,p}e^{i\delta^L_{n\rightarrow m, p}t}\right) \\
	%
...
\end{eqnarray}
{\it I note that this is equivalent to the Unager result.  I have no idea how I arrived at this result.  I somehow was able to pull things through the electric field operator.}
