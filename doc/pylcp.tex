...
To understand the units associated with $d\cdot E$, let us briefly consider the two level system.  The excited state decay rate is given by $\Gamma = k^3(d d^*+d^* d)/6 \pi\epsilon_0$.  Thus, $|d| = \sqrt{3 \pi \epsilon_0 \Gamma/k^3}$.  If $E_0 = \sqrt{2 I/\epsilon_0 c}$, the $\hbar d\cdot E = \sqrt{6 \pi  \Gamma/k^3 c}$.  This suggests that we should normalize $I$ to
\begin{equation}
	I_\text{sat} = 6\pi
\end{equation}
...
For the decay rate, we specify the 
